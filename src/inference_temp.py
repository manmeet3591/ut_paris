...
# Example usage:
data_dir = '/mnt/training_data_temp/'
...
loaded_model = netG
loaded_model.load_state_dict(torch.load('best_netG_temp.pth'))
loaded_model.eval()
...
input_tensor_ = input_tensor.contiguous().view(1, 6, 240, 240)


print('Reshaped input tensor shape: ', input_tensor_.shape)

original_shape_tensor = input_tensor_.reshape(1, 6, 240, 240)
...
predicted_sr_ = predicted_sr.cpu().numpy().reshape(1, 31, 240, 240)
...
predicted_sr_xr['TMP_2maboveground'] = (('time', 'latitude', 'longitude'), predicted_sr_[0,:,:,:])
...
predicted_sr_xr_inverse = xr.Dataset({var: inverse_min_max_normalize(predicted_sr_xr[var], global_min_target[var], global_max_target[var]) for var in predicted_sr_xr['TMP_2maboveground'].to_dataset()})
# print(predicted_sr_xr_inverse.APCP_surface.values)
#sys.exit()
predicted_sr_xr_inverse['TMP_2maboveground'].to_dataset().to_netcdf(file_path[:-3]+'_temppred.nc')
